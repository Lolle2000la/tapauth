...
assert_etc_tapauth() {
    ...
    # config.toml is generated by tmpfiles, not shipped, so it is deliberately
    # NOT declared in PKGBUILD's backup= array (pacman warns "backup entry file
    # not in package" for a non-shipped path). tmpfiles `f` preserves the file's
    # content on upgrades but re-asserts the configured owner/mode
    # (0644 tapauthd:tapauthd) on every systemd-tmpfiles --create, so config
    # edits survive while a manually changed mode/owner does not.
}
...
assert_removed() {
    ...
    # The generated config and state are not package-owned, so `pacman -R` leaves
    # them in place (AGENTS.md); only Debian's purge removes the generated config.
    check_file /etc/tapauth/config.toml "daemon-owned config survives pacman -R"
    if [ -d /var/lib/tapauth ]; then
        pass "/var/lib/tapauth survives pacman -R"
    else
        fail "/var/lib/tapauth removed by pacman -R"
    fi
}
