...
CONFIG_POLICY_PATH="/usr/share/polkit-1/actions/org.tapauth.config.admin.policy"
...
install_config_gui() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        if [[ -d /usr/share/polkit-1/actions ]]; then
            show_file_copy "tapauthd/org.tapauth.config.admin.policy" "$CONFIG_POLICY_PATH"
    ...
    if [[ -d /usr/share/polkit-1/actions ]]; then
        ...
        cp tapauthd/org.tapauth.config.admin.policy "$CONFIG_POLICY_PATH"
