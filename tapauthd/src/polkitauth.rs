...
/// Authorize an admin IPC call from a peer process.
///
/// Uses PolKit `CheckAuthorization` with the caller's process as the subject.
/// The `tapauthd` daemon must be registered as an action owner via the
/// `org.freedesktop.policykit.owner` annotation so it can query authorizations
/// for subjects belonging to other identities.
///
/// Falls back to UID==0 check when PolKit is unavailable (no system D-Bus).
pub async fn check_authorization(identity: &PeerIdentity) -> Result<(), String> {
    match check_polkit_authorization(identity).await {
        Ok(true) => Ok(()),
