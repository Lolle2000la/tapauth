...
class ReplayMitigationCache {
    ...
    companion object {
        ...
        @Volatile private var instance: ReplayMitigationCache? = null

        fun getInstance(): ReplayMitigationCache {
            return instance
                ?: synchronized(this) { instance ?: ReplayMitigationCache().also { instance = it } }
        }
    }
    ...
    fun isReplay(challenge: ByteArray, timestampUnixSeconds: Long): Boolean {
        ...
        if (timestampDelta > TIMESTAMP_VALIDITY_SECONDS) {
            Log.w(
                TAG,
                "Replay detected: Stale timestamp (delta=${timestampDelta}s, threshold=${TIMESTAMP_VALIDITY_SECONDS}s)",
            )
            return true
        }
        ...
        if (challengeCache.containsKey(challengeHex)) {
            Log.w(
                TAG,
                "Replay detected: Challenge nonce already seen (challenge=${challengeHex.take(16)}...)",
            )
            return true
        }
        ...
        Log.d(
            TAG,
            "Challenge accepted (cache size=${challengeCache.size}, challenge=${challengeHex.take(16)}...)",
        )
        return false
    }
    ...
    private fun cleanExpired() {
        ...
        if (sizeBefore != sizeAfter) {
            Log.d(
                TAG,
                "Cleaned ${sizeBefore - sizeAfter} expired entries from cache (remaining=${sizeAfter})",
            )
        }
    }
