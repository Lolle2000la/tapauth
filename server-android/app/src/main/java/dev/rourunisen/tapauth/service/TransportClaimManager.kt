 ...
 *
 * NOTE: This class used to be called `TransportLockManager`. It was renamed when
 * `TransportLockManager` became the Wi-Fi multicast-lock owner (power management), so the two
 * unrelated responsibilities no longer share a name.
 */
class TransportClaimManager private constructor() {

    private data class TransportClaim(val transport: TransportType, val timestamp: Long)

    // Map challenge (as hex string) to the transport that claimed it
    private val challengeLocks = ConcurrentHashMap<String, TransportClaim>()

    companion object {
        private const val TAG = "TransportClaimManager"

        // Locks expire after 2 minutes to prevent memory leaks
        private const val LOCK_EXPIRY_MS = 120_000L

        @Volatile private var instance: TransportClaimManager? = null

        fun getInstance(): TransportClaimManager {
            return instance
                ?: synchronized(this) { instance ?: TransportClaimManager().also { instance = it } }
        }
    }
    ...
    fun tryClaimTransport(challenge: ByteArray, transport: TransportType): Boolean {
        ...
        val existingLock = challengeLocks.putIfAbsent(challengeHex, TransportClaim(transport, now))
        ...
        if (now - existingLock.timestamp > LOCK_EXPIRY_MS) {
            ...
            val replaced =
                challengeLocks.replace(challengeHex, existingLock, TransportClaim(transport, now))
            if (replaced) {
                Log.d(
                    TAG,
