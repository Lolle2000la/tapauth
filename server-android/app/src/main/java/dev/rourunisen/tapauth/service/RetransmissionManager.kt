...
/**
 * Manages retransmission of AuthenticationGrant and AuthenticationDenial messages
 * according to authentication-flow.md Section 2.3.
 * 
 * Per specification:
...
class RetransmissionManager {
    ...
    companion object {
        ...
        fun getInstance(): RetransmissionManager {
            return instance ?: synchronized(this) {
                instance ?: RetransmissionManager().also { instance = it }
            }
        }
    }
     ...
     */
    fun startUdpRetransmission(
        scope: CoroutineScope,
        request: UdpRetransmissionRequest
    ) {
        ...
        val job = scope.launch {
            ...
            Log.d(TAG, "UDP retransmission completed for challenge ${challengeKey.take(16)}... (attempts=$attempts)")
        }
     ...
     */
    fun startBleRetransmission(
        scope: CoroutineScope,
        request: BleRetransmissionRequest
    ) {
        val challengeKey = request.challenge.toHex()
        ...
        val job = scope.launch {
            ...
            // Clean up
            activeRetransmissions.remove(challengeKey)
            Log.d(TAG, "BLE retransmission completed for challenge ${challengeKey.take(16)}... (attempts=$attempts)")
        }
    ...
    /**
     * Stop retransmission for a specific challenge
     * Call this when GrantConfirmation is received
     */
    ...
    fun getStats(): Map<String, Any> {
        return mapOf(
            "active_retransmissions" to activeRetransmissions.size
        )
    }
}
