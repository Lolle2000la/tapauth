...
/**
 * Thread-safe manager to ensure only one transport channel (UDP or BLE) is used
 * per authentication session. This prevents duplicate responses when both channels
 * are available.
 * 
...
class TransportLockManager private constructor() {
    ...
    private data class TransportLock(
    ...
    companion object {
        ...
        @Volatile
        private var instance: TransportLockManager? = null
        
        fun getInstance(): TransportLockManager {
            return instance ?: synchronized(this) {
                instance ?: TransportLockManager().also { instance = it }
            }
        }
    }
    
    /**
     * Try to claim this challenge for the given transport.
     * Returns true if successful (first to claim), false if already claimed by another transport.
     */
    fun tryClaimTransport(challenge: ByteArray, transport: TransportType): Boolean {
        ...
        val existingLock = challengeLocks.putIfAbsent(
            challengeHex,
            TransportLock(transport, now)
        )
        ...
        if (now - existingLock.timestamp > LOCK_EXPIRY_MS) {
            ...
            val replaced = challengeLocks.replace(
                challengeHex,
                existingLock,
                TransportLock(transport, now)
            )
            if (replaced) {
                Log.d(TAG, "Transport $transport claimed expired challenge ${challengeHex.take(16)}...")
                return true
            }
        }
        ...
        if (existingLock.transport != transport) {
            Log.i(TAG, "Transport $transport blocked - challenge ${challengeHex.take(16)}... already claimed by ${existingLock.transport}")
            return false
        }
    ...
    fun releaseLock(challenge: ByteArray) {
        ...
        if (removed != null) {
            Log.d(TAG, "Released lock for challenge ${challengeHex.take(16)}... (transport: ${removed.transport})")
        }
    }
