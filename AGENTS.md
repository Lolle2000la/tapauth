...
- The distro packages and `install.sh` **auto-add existing interactive local users** (UID ≥ 1000, real login shell — excluding `nologin`/`false`) to `tapauthd-clients` on install/upgrade (users created later are picked up on the next package upgrade). The step is guarded (`getent group`, `command -v usermod`), idempotent and non-fatal. It only takes effect after the user re-logs (or the process re-initialises its groups), and it only matters for user-session components/ the opt-in `pam_fprintd.so` emulation path — root greeters/auth helpers are unaffected. Membership is deliberately **not** removed on uninstall/purge (only the group itself is, on purge).
...
- User-session lockers (e.g. KDE `kscreenlocker_worker`, which runs as the logged-in user) reach `tapauthd` over `/run/tapauthd/tapauthd.sock` and therefore need membership in `tapauthd-clients`; the packages/`install.sh` add existing interactive users automatically (a re-login is required for it to take effect)
